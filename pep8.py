...
def extended_blank_lines(logical_line,
    ...
    if previous_logical.startswith('class '):
        if (
            ...
            DOCSTRING_REGEX.match(logical_line)
        ):
        ...
        if blank_lines and DOCSTRING_REGEX.match(logical_line):
            yield (0, 'E303 too many blank lines ({0})'.format(blank_lines))
    elif DOCSTRING_REGEX.match(previous_logical):
...
def register_check(check, codes=None):
    ...
    if inspect.isfunction(check):
        args = inspect.signature(check)
        if args and args in ('physical_line', 'logical_line'):
            if codes is None:
                codes = ERRORCODE_REGEX.findall(check.__doc__ or '')
            _add_check(check, args[0], codes, args)
    elif inspect.isclass(check):
        if iinspect.signature(check.__init__) == ['self', 'tree']:
